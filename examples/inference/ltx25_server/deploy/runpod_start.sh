#!/usr/bin/env bash
# RunPod (or any systemd-less container) entrypoint for the LTX-2.5 server.
#
...
# equivalent of enabling ltx25@.service on a VM.
#
...
#   CONFIG   server config on the persistent volume  [/workspace/ltx25/config.yaml]
#   GPU      GPU index inside the pod                 [0]
#   PORT     listen port                              [8000]
#   LOG_DIR  supervisor + server logs (persist it!)   [/workspace/ltx25/logs/gpu$GPU]
set -euo pipefail

export CONFIG="${CONFIG:-/workspace/ltx25/config.yaml}"
export GPU="${GPU:-0}"
export PORT="${PORT:-8000}"
export LOG_DIR="${LOG_DIR:-/workspace/ltx25/logs/gpu${GPU}}"
