#!/usr/bin/env python3
"""LTX-2.5 production HTTP server (compiled, first-frame / first+last-frame).

    env -u LD_LIBRARY_PATH python server.py --config config.yaml

Serves the validated LTX-2.5 two-stage distilled recipe (see
examples/inference/basic/basic_ltx2_5_i2av_two_stage.py): CFG++ ancestral
sampling at half resolution, x2 latent upsample, then a 3-step refine at
the requested FINAL resolution, with joint audio.
...
Endpoints:
    ...
    POST /v1/generate_s3  same, but uploads HQ (+ optional LQ) to S3
...
served combo is reported in X-LTX25-* response headers.

Reliability: every request gets an id (X-LTX25-Request-Id) and a JSON
...
This is a self-contained fork of examples/inference/ltx23_server/server.py
(which serves live traffic and must stay untouched); the operational
semantics are identical, only the recipe differs — see ltx25_engine.py.
"""
...
# ltx25_engine keeps all torch/fastvideo imports function-local.
from ltx25_engine import (
    GenerationRequest,
    Ltx25ServerConfig,
    build_s3_key,
    create_generator,
...
# The tail anchor stays OUT of the stage-2 refine pass by default — the
# opposite of the 2.3 server's product default, because the validated 2.5
# example keeps it out (it misbehaved in ComfyUI testing). Per-request
# overridable via the last_in_upscale form field.
_DEFAULT_LAST_IN_UPSCALE = False

# Indirection so tests can intercept the supervisor-restart exit.
_terminate = lambda: os._exit(1)  # noqa: E731


def setup_request_logging(cfg: Ltx25ServerConfig) -> logging.Logger:
    """JSON-lines request log: always mirrored to stdout; also rotated
    into <log_dir>/requests.jsonl when log_dir is set."""
    logger = logging.getLogger("ltx25.requests")
...
def build_app(generator, cfg: Ltx25ServerConfig, s3_client=None) -> FastAPI:
    app = FastAPI(title="FastVideo LTX-2.5 server", version="1.0")
    ...
    def modes() -> dict:
        # width/height are the FINAL size; stage 1 renders at half of it.
        return {
            "modes": [{
                "width": m.width,
                "height": m.height,
                ...
                "fps": m.fps,
                "stage1_width": m.stage1_size()[0],
                "stage1_height": m.stage1_size()[1],
            } for m in cfg.modes]
        }
    ...
    def _run_generation(
        ...
        *,
        ...
        id_header = {"X-LTX25-Request-Id": request_id}
        ...
        if not 0.0 <= image_crf <= 51.0:  # H.264 CRF range (LTXVPreprocess)
            raise bail(400, "image_crf must be in [0, 51]")
        ...
        record["params"]["stage1"] = list(mode.stage1_size())
        record["params"]["exact_match"] = exact

        workdir = Path(tempfile.mkdtemp(prefix="ltx25_req_", dir=scratch_root))
        try:
    ...
    def generate(
        ...
        last_in_upscale: bool = Form(_DEFAULT_LAST_IN_UPSCALE),
        # Conditioning CRF (LTXVPreprocess img_compression, default 38).
        # Both stages reuse it — the 2.5 recipe has no separate stage-2 CRF.
        ...
        return FileResponse(
            ...
            filename="output.mp4",
            headers={
                "X-LTX25-Width": str(mode.width),
                "X-LTX25-Height": str(mode.height),
                "X-LTX25-Num-Frames": str(mode.num_frames),
                "X-LTX25-Fps": str(mode.fps),
                "X-LTX25-Exact-Match": "1" if exact else "0",
                "X-LTX25-Seed": str(req_seed),
                "X-LTX25-Generate-Seconds": f"{result['gen_seconds']:.2f}",
                "X-LTX25-Encode-Seconds": f"{encode_seconds:.2f}",
                **id_header,
            },
...
def main() -> None:
    ...
    if cfg.warmup_on_start and not args.skip_warmup:
        ...
        threading.Thread(target=_warmup, name="ltx25-warmup", daemon=True).start()
    else:
        app.state.ready.set()
